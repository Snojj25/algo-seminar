...
For the lifted graph $G'$ defined above and operated under the update protocol of Section~\ref{sec:reduction:lifting}, the number of length-3 \emph{walks} from $u \in L_1$ to $v \in L_4$ in $G'$ equals the number of length-3 paths from $u$ to $v$ in $G$.
\end{theorem}

\begin{proof}
%% F3: emphasize the load-bearing update protocol.
%% Forward: any path in G lifts to a layered walk.
%% Reverse: rule out coincident vertices using (i) no self-loops in lifted graph,
%% (ii) edge (u,v) absent from A,B,C at query time by the D-first/A-last protocol.
%% Cite \cite{AssadiShah25} for the original argument.
\todo{proof body}
\end{proof}
...
\begin{lemma}[Update-time accounting for Table~\ref{tab:datastructures}; \cite{AssadiShah25}]
...
\todo{one paragraph: aggregation breaks once $A$ updates; the paper introduces phases of size $m^{1-\delta}$ that allow FMM precomputation on the (now-frozen) previous phase. Constraint Eq~(9) of \cite{AssadiShah25} forces $\omega < 5/2 - O(\varepsilon)$, which is why Strassen ($\omega \approx 2.81$) is not sufficient. State the headline result we do \emph{not} re-prove:}
...
\todo{closing note: ``We restate Theorem~\ref{thm:main-paper} verbatim from \cite{AssadiShah25}; its proof requires combining the warm-up of Section~\ref{sec:warmup}, the phases machinery sketched above, and removal of the simplifying assumptions (paper Sections 6--7), which is beyond our scope.''}
